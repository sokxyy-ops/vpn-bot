...
def plan_badge_id(plan: str) -> str:
    return EMOJI_ID_STANDARD if plan == "standard" else EMOJI_ID_FAMILY


def plan_badge_fallback(plan: str) -> str:
    return "🟩" if plan == "standard" else "🟦"


def html_plan_badge(plan: str) -> str:
    return tg_emoji_html(plan_badge_id(plan), plan_badge_fallback(plan))
...
def plan_meta(plan: str):
    ...
    if plan == "standard":
        return f"{html_plan_badge('standard')} Стандарт", "👤 1 пользователь • 📱 до 3 устройств", "3", price_standard
    return f"{html_plan_badge('family')} Семейная", "👥 до 8 пользователей • 📱 до 3 устройств каждому", "3", price_family


# ================== UI TEXT ==================
def text_menu():
    return (
        f"{tg_emoji_html(EMOJI_ID_GLOBE, '🌐')} <b>SkyWhy VPN</b>\n\n"
...
def text_buy_intro():
    ...
    return (
        f"{tg_emoji_html(EMOJI_ID_TARIFF, '🛍')} <b>Выбор тарифа</b>\n\n"
...
def text_subscription_card(from_user, subs: Optional[list]):
    ...
    for idx, sub in enumerate(subs, start=1):
        _plan_name, conditions, device_limit, _amount = plan_meta(sub["plan"])
        parts.extend([
            "",
...
def kb_after_issue(plan: Optional[str] = None):
    ...
    if plan in ("standard", "family"):
        rows.append([InlineKeyboardButton(text=pretty_plan_name(plan), callback_data=f"sub:key:{plan}", icon_custom_emoji_id=plan_badge_id(plan))])
...
def kb_sub_with_refresh(user_id: int):
    ...
    for sub in subs:
        ...
        if plan in ("standard", "family"):
            plan_buttons.append(
                InlineKeyboardButton(text=pretty_plan_name(plan), callback_data=f"sub:key:{plan}", icon_custom_emoji_id=plan_badge_id(plan))
            )
...
async def send_banner_or_text(chat_id: int, text: str, reply_markup=None):
    try:
        if os.path.exists(BANNER_PATH):
            await bot.send_photo(chat_id, FSInputFile(BANNER_PATH), caption=text, reply_markup=reply_markup, parse_mode="HTML")
        else:
