...
async def admin_write_user(call: CallbackQuery, state: FSMContext):
    await call.answer()
    user_id = int(call.data.split(":")[1])
    await state.update_data(target_user=user_id)
    await state.set_state(AdminStates.message_user_wait)
    await call.message.answer("✉️ Введите сообщение для пользователя:")

@dp.message(AdminStates.message_user_wait)
async def admin_send_user(message: Message, state: FSMContext):
    data = await state.get_data()
