...
def db_init():
    ...
    """)
    _add_column_if_missing(con, "trial_subscriptions", "plan", "ALTER TABLE trial_subscriptions ADD COLUMN plan TEXT NOT NULL DEFAULT 'standard'")
    _add_column_if_missing(con, "trial_subscriptions", "created_at", "ALTER TABLE trial_subscriptions ADD COLUMN created_at INTEGER NOT NULL DEFAULT 0")
    con.commit()
...
def db_get_trial_by_user(user_id: int):
    con = db()
    try:
        cur = con.cursor()
        cur.execute("""
            SELECT user_id, token, plan, expires_at, created_at
...
def db_upsert_trial(user_id: int, token: str, plan: str, expires_at: int):
    con = db()
    try:
        cur = con.cursor()
        cur.execute("""
            INSERT INTO trial_subscriptions(user_id, token, plan, expires_at, created_at)
            VALUES(?,?,?,?,?)
            ON CONFLICT(user_id) DO UPDATE SET
        ...
        """, (user_id, token, plan, int(expires_at), int(time.time())))
        con.commit()
    finally:
        con.close()
...
def kb_main(user_id: int):
    ...
    rows = [
        ...
        [ibtn(text="🎁 Пробная подписка", callback_data="menu:trial", style=BTN_PRIMARY_STYLE)],
...
def kb_admin_user_view(user_id: int, mode: str, page: int):
    ...
    buttons.append([InlineKeyboardButton(text="🎁 Сбросить пробник", callback_data=f"admin:resettrial:{user_id}:{mode}:{page}")])
...
# ================== TRIAL ==================
def text_trial_intro() -> str:
    return (
        "🎁 <b>Пробная подписка</b>\n\n"
        "• Основа: <b>Стандартная</b>\n"
        "• Срок действия: <b>1 час</b>\n"
        "• Формат: <b>подписка Happ</b>\n"
        "• После окончания срока подписка станет пустой\n"
        "• Если пробник уже активирован, бот пришлёт ту же ссылку повторно\n\n"
        "Нажми кнопку ниже, чтобы получить пробную подписку."
    )


def kb_trial_get() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎁 Получить пробник на 1 час", callback_data="trial:get")],
        [InlineKeyboardButton(text="⬅️ Назад в меню", callback_data="menu:main")]
    ])


def trial_sign(token: str) -> str:
    return hashlib.sha256(f"{token}:{TRIAL_SHARED_SECRET}".encode("utf-8")).hexdigest()


def _http_json(url: str, payload: dict | None = None, method: str = "GET", headers: dict | None = None, timeout: int = 20) -> dict:
    data = None
    req_headers = {"User-Agent": "SkyWhyBot/1.0", "Accept": "application/json, text/plain, */*"}
    if headers:
        req_headers.update(headers)
    if payload is not None:
        data = json.dumps(payload).encode("utf-8")
        req_headers.setdefault("Content-Type", "application/json")
    req = urllib_request.Request(url=url, data=data, headers=req_headers, method=method.upper())
    with urllib_request.urlopen(req, timeout=timeout) as resp:
        raw = resp.read().decode("utf-8", errors="replace")
        return json.loads(raw) if raw else {}


def build_trial_link(token: str) -> str:
    return f"{SUB_BASE_URL}/trial/{token}"


def format_left_seconds(left: int) -> str:
    left = max(0, int(left))
    h = left // 3600
    m = (left % 3600) // 60
    s = left % 60
...
def try_build_limited_link(raw_url: str) -> str:
    ...
    try:
        ...
        data = _http_json("https://api.happ-proxy.com/api/add-install", payload=payload, method="POST")
        install_code = str(data.get("install_code") or data.get("code") or "").strip()
        if install_code:
            sep = "&" if "?" in raw_url else "?"
            return f"{raw_url}{sep}installid={install_code}"
    except Exception as e:
        print("[trial] limited-link fallback:", e)
    return raw_url


async def send_trial_link_message(target: Message | CallbackQuery, user_id: int):
    now = int(time.time())
    trial = db_get_trial_by_user(user_id)
    if trial and int(trial.get("expires_at") or 0) > now:
        token = trial["token"]
        expires_at = int(trial["expires_at"])
    else:
        token = secrets.token_urlsafe(24)
        expires_at = now + TRIAL_DURATION_SEC
        db_upsert_trial(user_id, token, "standard", expires_at)

    raw_url = build_trial_link(token)
    final_url = try_build_limited_link(raw_url)
    left = format_left_seconds(expires_at - now)

    text = (
        "🎁 <b>Пробная подписка активна</b>\n\n"
        "📦 Основа: <b>Стандартная</b>\n"
        "⏳ Срок действия: <b>1 час</b>\n"
        f"⌛ Осталось: <b>{left}</b>\n"
        "📱 Лимит устройств: <b>1 устройство</b>\n"
        "🔗 Формат: <b>Happ subscription link</b>\n\n"
        "Добавляй ссылку в Happ как подписку. После окончания часа подписка станет пустой и серверы пропадут после обновления.\n\n"
        f"<code>{html_escape(final_url)}</code>"
    )
...
async def reply_trial_btn(m: Message):
    await m.answer(text_trial_intro(), reply_markup=kb_trial_get(), parse_mode="HTML")
    await m.answer("Нижнее меню 👇", reply_markup=kb_reply_menu(m.from_user.id))
...
async def http_trial_info(request: web.Request):
    try:
        token = (request.query.get("token") or "").strip()
        sign = (request.query.get("sign") or "").strip()

        if not token or not sign:
            return web.json_response({"ok": False, "reason": "bad_request"}, status=400)

        if not TRIAL_SHARED_SECRET:
            return web.json_response({"ok": False, "reason": "secret_missing"}, status=500)

        expected = trial_sign(token)
        if sign != expected:
            return web.json_response({"ok": False, "reason": "bad_sign"}, status=403)
        ...
        now = int(time.time())
        expires_at = int(trial["expires_at"] or 0)
        if not expires_at or now >= expires_at:
            return web.json_response({
                "ok": False,
                "reason": "expired",
                "plan": str(trial.get("plan") or "standard"),
        ...
        print("[trial-info] error:", e)
        return web.json_response({"ok": False, "reason": f"server_error: {e}"}, status=500)


async def start_http_server() -> tuple[web.AppRunner, web.TCPSite]:
    app = web.Application()
    app.router.add_get("/", healthcheck_handler)
    app.router.add_get("/trial-info", http_trial_info)
...
async def menu_router(call: CallbackQuery):
    try:
        ...
        if action == "trial":
            await call.message.answer(text_trial_intro(), reply_markup=kb_trial_get(), parse_mode="HTML")
            await refresh_reply_menu(call.message.chat.id, call.from_user.id)
            return
...
async def trial_get(call: CallbackQuery):
    try:
        await send_trial_link_message(call, call.from_user.id)
    except Exception as e:
        await call.message.answer(f"⚠️ Ошибка при выдаче пробника: {html_escape(str(e))}", parse_mode="HTML")
    finally:
        try:
            await call.answer()
        except Exception:
...
async def cmd_resettrial(m: Message):
    if not is_admin(m.from_user.id):
        return
    parts = (m.text or "").split()
    if len(parts) < 2 or not parts[1].isdigit():
        await m.answer("Используй: /resettrial USER_ID")
        return
    uid = int(parts[1])
    deleted = db_delete_trial(uid)
    await m.answer(f"🎁 Сброс пробника для {uid}: {deleted}")


@dp.callback_query(F.data.startswith("admin:resettrial:"))
async def admin_resettrial(call: CallbackQuery):
    try:
        _, _, _, user_id, mode, page = call.data.split(":")
        user_id_i = int(user_id)
        deleted = db_delete_trial(user_id_i)
        await call.answer("Пробник сброшен", show_alert=True)
        user = db_get_user(user_id_i)
        text = build_admin_user_card_text(user) if user else "Пользователь не найден"
        await call.message.edit_text(
            text,
            reply_markup=kb_admin_user_view(user_id_i, mode, int(page)),
            parse_mode="Markdown"
        )
    except Exception as e:
