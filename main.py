...
def build_happ_connect_url(raw_value: Optional[str]) -> Optional[str]:
    value = (raw_value or "").strip()
    if not value:
        return None

    lowered = value.lower()
    if lowered.startswith("happ://"):
        return value

    supported_direct_links = (
        "https://", "http://",
        "vless://", "vmess://", "trojan://",
        "ss://", "ssr://", "hy2://", "hysteria://",
        "tuic://", "wg://", "wireguard://",
    )
    if lowered.startswith(supported_direct_links):
        return f"happ://add/{quote(value, safe='')}"

    return None


def get_connect_url_for_plan(plan: str) -> Optional[str]:
    latest_value = get_latest_key_for_plan(plan)
    return build_happ_connect_url(latest_value)
...
def kb_main(user_id: int):
    active = db_get_active_order(user_id)
    rows = [
        [InlineKeyboardButton(text="Купить подписку", callback_data="menu:buy", icon_custom_emoji_id=EMOJI_IDS["buy"])],
        [InlineKeyboardButton(text="Моя подписка", callback_data="menu:sub", icon_custom_emoji_id=EMOJI_IDS["my_sub"])],
    ...
    rows.extend([
        [InlineKeyboardButton(text="Наш канал", url=TG_CHANNEL, icon_custom_emoji_id=EMOJI_IDS["channel"])],
    ])
...
def kb_buy():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"Стандарт • {std_price}₽", callback_data="buy:standard", icon_custom_emoji_id=EMOJI_IDS["standard"])],
        [InlineKeyboardButton(text="1 пользователь • до 3 устройств", callback_data="noop")],
        [InlineKeyboardButton(text=f"Семейная • {fam_price}₽", callback_data="buy:family", icon_custom_emoji_id=EMOJI_IDS["family"])],
...
def kb_after_issue(plan: Optional[str] = None):
    ...
    if plan in ("standard", "family"):
        connect_url = get_connect_url_for_plan(plan)
        if connect_url:
            rows.append([InlineKeyboardButton(text="📲 Подключить в Happ", url=connect_url)])
...
def kb_sub_no_sub(user_id: int):
    rows = [[InlineKeyboardButton(text="Купить подписку", callback_data="menu:buy", icon_custom_emoji_id=EMOJI_IDS["buy"])]]
        ...
        if plan in ("standard", "family"):
            row = [
                InlineKeyboardButton(
                    text=("Стандартная" if plan == "standard" else "Семейная"),
                    callback_data=f"sub:key:{plan}",
                    icon_custom_emoji_id=EMOJI_IDS["standard"] if plan == "standard" else EMOJI_IDS["family"]
                )
            ]
            connect_url = get_connect_url_for_plan(plan)
            if connect_url:
                row.append(InlineKeyboardButton(text="📲 Подключить", url=connect_url))
            rows.append(row)
...
async def send_key_to_user(user_id: int, plan: str, key: str):
    ...
    await bot.send_message(
        ...
        "Нажми кнопку *📲 Подключить в Happ* ниже, чтобы сразу добавить подписку.\n"
        "Или нажми кнопку с тарифом, если хочешь посмотреть сам ключ.\n"
        "На телефоне можно нажать на код или зажать его, чтобы скопировать.\n\n"
    ...
    try:
        ...
        if connect_url:
            reply_markup = InlineKeyboardMarkup(
                inline_keyboard=[[InlineKeyboardButton(text="📲 Подключить в Happ", url=connect_url)]]
            )
