...
async def trial_get(call: CallbackQuery):
    try:
        if db_trial_used(call.from_user.id):
            ...
            if trial and int(trial.get("expires_at") or 0) > int(time.time()):
                await call.answer("Пробник уже активирован", show_alert=True)
                await call.message.answer(
                    "🎁 Пробный доступ уже был выдан ранее.\n\n"
                    f"Он действует только 1 час. Осталось: *{md_escape(trial_time_left_text(trial['expires_at']))}*",
                    parse_mode="Markdown"
                )
            else:
        ...
        expires_at = int(time.time()) + TRIAL_DURATION_SEC
        db_create_or_replace_trial(call.from_user.id, token, expires_at)

        try:
            crypt_link = await asyncio.to_thread(build_trial_happ_link, token)
        except Exception as e:
            await call.message.answer(
                "⚠️ Не удалось собрать зашифрованную ссылку Happ crypt5.\n"
                f"Детали: `{md_escape(str(e))}`"
            )
            return
