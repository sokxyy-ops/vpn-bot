...
BTN_OK_STYLE = "success"
BTN_DANGER_STYLE = "danger"
BTN_NAV_STYLE = "default"
...
def ibtn(
    ...
    **extra: Any,
    ...
    payload: Dict[str, Any] = {"text": text, "style": style}
...
class PgCursorAdapter:
    ...
    def execute(self, sql: str, params: Optional[tuple] = None):
        clean = sql.strip()
        upper = clean.upper()
        if upper.startswith("PRAGMA JOURNAL_MODE") or upper.startswith("PRAGMA SYNCHRONOUS") or upper.startswith("PRAGMA CACHE_SIZE") or upper.startswith("PRAGMA TEMP_STORE"):
            self._rows = []
            self.rowcount = 0
            return self

        m = re.match(r"PRAGMA\s+table_info\(([^)]+)\)", clean, flags=re.I)
        if m:
            ...
            self._rows = [(0, r[0], "", 0, None, 0) for r in rows]
            self.rowcount = len(self._rows)
            return self

        converted = sql
        converted = converted.replace("INTEGER PRIMARY KEY AUTOINCREMENT", "BIGSERIAL PRIMARY KEY")
        converted = converted.replace("INTEGER PRIMARY KEY", "BIGINT PRIMARY KEY")
        converted = converted.replace(" INTEGER DEFAULT 0", " BIGINT DEFAULT 0")
        converted = converted.replace(" INTEGER NOT NULL", " BIGINT NOT NULL")
        converted = converted.replace(" INTEGER,", " BIGINT,")
        converted = converted.replace(" INTEGER\n", " BIGINT\n")
        converted = converted.replace(" VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", " VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)")
        converted = converted.replace(" VALUES(?,?,?,?,?)", " VALUES(%s,%s,%s,%s,%s)")
        converted = converted.replace(" VALUES(?,?)", " VALUES(%s,%s)")
        converted = converted.replace(" VALUES(?)", " VALUES(%s)")
        converted = re.sub(r"\?", "%s", converted)

        if converted.strip().upper().startswith("INSERT OR IGNORE INTO"):
            converted = converted.replace("INSERT OR IGNORE INTO", "INSERT INTO", 1)
            if "profit_meta" in converted:
                converted += " ON CONFLICT (key) DO NOTHING"
            elif "keys_store" in converted:
                converted += " ON CONFLICT (plan, key) DO NOTHING"

        self.cur.execute(converted, params or ())
...
def migrate_sqlite_to_postgres_if_needed():
    if not (USE_POSTGRES and DATABASE_URL and os.path.exists(SQLITE_MIGRATION_PATH) and psycopg2 is not None):
        return
    ...
    regs = cur.fetchone()
    if not regs or not regs[0] or not regs[1]:
        pg.close()
        return
    cur.execute("SELECT COUNT(*) FROM users")
    users_count = int(cur.fetchone()[0] or 0)
    cur.execute("SELECT COUNT(*) FROM orders")
    orders_count = int(cur.fetchone()[0] or 0)
    if users_count or orders_count:
        pg.close()
        return

    src = sqlite3.connect(SQLITE_MIGRATION_PATH)
    scur = src.cursor()

    def rows_for(table):
        try:
            scur.execute(f"SELECT * FROM {table}")
            return scur.fetchall(), [d[0] for d in scur.description]
        except Exception:
            return [], []

    rows, cols = rows_for("users")
    for row in rows:
        ...
        cur.execute(
            ...
            """,
            (item.get("user_id"), item.get("username") or "", item.get("first_name") or "", item.get("last_seen") or 0, item.get("is_blocked") or 0, item.get("is_banned") or 0)
        )

    rows, cols = rows_for("orders")
    for row in rows:
        item = dict(zip(cols, row))
        cur.execute(
            """
            INSERT INTO orders(id, user_id, username, plan, amount, status, created_at, payment_msg_id, issued_key, accepted_at, admin_msg_id, resend_count, last_resend_at, payment_provider_id, payment_url)
            VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            ON CONFLICT (id) DO NOTHING
            """,
            (item.get("id"), item.get("user_id"), item.get("username") or "", item.get("plan"), item.get("amount") or 0, item.get("status") or "", item.get("created_at") or 0, item.get("payment_msg_id"), item.get("issued_key"), item.get("accepted_at"), item.get("admin_msg_id"), item.get("resend_count") or 0, item.get("last_resend_at") or 0, item.get("payment_provider_id"), item.get("payment_url"))
        )

    rows, cols = rows_for("settings")
    for row in rows:
        item = dict(zip(cols, row))
        cur.execute("INSERT INTO settings(key, value) VALUES(%s,%s) ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value", (item.get("key"), item.get("value") or ""))

    rows, cols = rows_for("keys_store")
    for row in rows:
        item = dict(zip(cols, row))
        cur.execute(
            """
            INSERT INTO keys_store(id, plan, key, used, used_at, order_id)
            VALUES(%s,%s,%s,%s,%s,%s)
            ON CONFLICT (id) DO NOTHING
            """,
            (item.get("id"), item.get("plan"), item.get("key"), item.get("used") or 0, item.get("used_at"), item.get("order_id"))
        )

    rows, cols = rows_for("profit_meta")
    for row in rows:
        item = dict(zip(cols, row))
        cur.execute("INSERT INTO profit_meta(key, value) VALUES(%s,%s) ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value", (item.get("key"), item.get("value") or "0"))

    cur.execute("SELECT setval(pg_get_serial_sequence('orders','id'), COALESCE((SELECT MAX(id) FROM orders), 1), true)")
    cur.execute("SELECT setval(pg_get_serial_sequence('keys_store','id'), COALESCE((SELECT MAX(id) FROM keys_store), 1), true)")
    pg.commit()
    src.close()
    pg.close()
...
SQLITE_MIGRATION_PATH = os.getenv("SQLITE_MIGRATION_PATH", DB_PATH).strip()
...
def db():
    if USE_POSTGRES and DATABASE_URL:
        if psycopg2 is None:
            raise RuntimeError("psycopg2 is required when USE_POSTGRES=1")
...
def db_create_order(user_id: int, username: Optional[str], plan: str, amount: int):
    ...
    if USE_POSTGRES and DATABASE_URL:
        ...
        order_id = int(cur.cur.fetchone()[0])
    else:
        cur.execute("""
            INSERT INTO orders(
                user_id, username, plan, amount, status, created_at,
...
def kb_reply_menu(user_id: int):
    ...
    if is_admin(user_id):
        ...
        return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True, input_field_placeholder="SkyWhy admin")

    rows = [
        [KeyboardButton(text="🏠 Меню"), KeyboardButton(text="📦 Моя подписка")],
        [KeyboardButton(text="🛒 Купить"), KeyboardButton(text="📢 Канал")],
    ]
    if active:
        rows.append([KeyboardButton(text="❌ Отменить заказ")])

    return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True, input_field_placeholder="SkyWhy VPN")
...
def kb_main(user_id: int):
    ...
    rows = [
        ...
        [ibtn(text="Моя подписка", callback_data="menu:sub", style=BTN_INFO_STYLE, icon_custom_emoji_id=EMOJI_IDS["my_sub"])],
        [ibtn(text="ℹ️ Информация", callback_data="menu:info", style=BTN_NAV_STYLE)],
    ]
    ...
    rows.extend([
        [ibtn(text="Наш канал", url=TG_CHANNEL, style=BTN_NAV_STYLE, icon_custom_emoji_id=EMOJI_IDS["channel"])],
    ])
...
def kb_buy():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [ibtn(text="⬅️ Назад в меню", callback_data="menu:main", style=BTN_NAV_STYLE)],
    ])
...
def kb_agreement(plan: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [ibtn(text="✅ Согласен, продолжить", callback_data=f"agree:{plan}", style=BTN_OK_STYLE)],
        [ibtn(text="⬅️ К тарифам", callback_data="menu:buy", style=BTN_NAV_STYLE)],
    ])
...
def kb_payment(order_id: int, payment_url: Optional[str] = None):
    ...
    rows.append([ibtn(text="🔄 Проверить оплату", callback_data=f"checkpay:{order_id}", style=BTN_OK_STYLE)])
    rows.append([ibtn(text="❌ Отменить заказ", callback_data=f"cancel:{order_id}", style=BTN_DANGER_STYLE)])
    rows.append([ibtn(text="🏠 В меню", callback_data="menu:main", style=BTN_NAV_STYLE)])
    return InlineKeyboardMarkup(inline_keyboard=rows)

def kb_admin_decision(order_id: int):
    order = db_get_order(order_id)
    keyboard = [
        [ibtn(text="✅ Принять", callback_data=f"admin:ok:{order_id}", style=BTN_OK_STYLE),
...
def kb_after_issue(plan: Optional[str] = None):
    ...
    if plan in ("standard", "family"):
        rows.append([InlineKeyboardButton(text=("Стандартная" if plan == "standard" else "Семейная"), callback_data=f"sub:key:{plan}", icon_custom_emoji_id=EMOJI_IDS["standard"] if plan == "standard" else EMOJI_IDS["family"])])

    rows.extend([
        [ibtn(text="📱 Android", url=HAPP_ANDROID_URL, style=BTN_NAV_STYLE),
         ibtn(text="🍎 iPhone", url=HAPP_IOS_URL, style=BTN_NAV_STYLE)],
        [ibtn(text="💻 Windows", url=HAPP_WINDOWS_URL, style=BTN_NAV_STYLE)],
        [ibtn(text="🔒 Приватная группа", url=PRIVATE_GROUP_LINK, style=BTN_INFO_STYLE)],
        [ibtn(text="⭐ Оставить отзыв", url=REVIEW_LINK, style=BTN_OK_STYLE)],
        [ibtn(text="🏠 В меню", callback_data="menu:main", style=BTN_NAV_STYLE)],
    ])
    return InlineKeyboardMarkup(inline_keyboard=rows)

def kb_require_subscription():
    return InlineKeyboardMarkup(inline_keyboard=[
        [ibtn(text="📣 Подписаться на канал", url=TG_CHANNEL, style=BTN_NAV_STYLE)],
        [ibtn(text="✅ Проверить подписку", callback_data="checksub", style=BTN_OK_STYLE)],
    ])
...
def kb_admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [ibtn(text="📦 Заказы", callback_data="admin:list", style=BTN_INFO_STYLE),
         ibtn(text="🔎 Найти заказ", callback_data="admin:search", style=BTN_NAV_STYLE)],
        [ibtn(text="👥 Пользователи", callback_data="admin:users:0", style=BTN_INFO_STYLE),
         ibtn(text="⛔ Баны", callback_data="admin:banned:0", style=BTN_DANGER_STYLE)],
        [ibtn(text="💰 Прибыль", callback_data="admin:profit", style=BTN_OK_STYLE),
         ibtn(text="📢 Рассылка", callback_data="admin:broadcast", style=BTN_NAV_STYLE)],
        [ibtn(text="🏷 Цены", callback_data="admin:prices", style=BTN_INFO_STYLE),
         ibtn(text="🔑 Ключи", callback_data="admin:keys", style=BTN_NAV_STYLE)],
    ])
...
def kb_admin_list(rows):
    ...
    keyboard.append([
        ibtn(text="🔄 Обновить", callback_data="admin:list", style=BTN_OK_STYLE),
        ibtn(text="⬅️ Назад", callback_data="admin:home", style=BTN_NAV_STYLE)
    ])
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
...
def kb_admin_profit():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [ibtn(text="⬅️ Админ", callback_data="admin:home", style=BTN_NAV_STYLE)],
    ])
...
def kb_admin_users_page(rows, page: int, total: int, mode: str):
    ...
    keyboard.append([ibtn(text="⬅️ Админ", callback_data="admin:home", style=BTN_NAV_STYLE)])
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
...
async def buy_from_reply(m: Message):
    await m.answer(text_buy_intro(), reply_markup=kb_buy(), parse_mode="HTML")
    await refresh_reply_menu(m.chat.id, m.from_user.id)


@dp.message(F.text == "📢 Канал")
async def channel_from_reply(m: Message):
    await m.answer(f"📢 Наш канал: {TG_CHANNEL}", reply_markup=kb_main(m.from_user.id))
    await refresh_reply_menu(m.chat.id, m.from_user.id)


@dp.message(F.text == "⚙️ Админ")
async def admin_from_reply(m: Message):
    if not is_admin(m.from_user.id):
        return
    await m.answer("🛠 *Админ-панель*\nВыбери раздел 👇", reply_markup=kb_admin_menu())
    await refresh_reply_menu(m.chat.id, m.from_user.id)
...
async def admin_orders_from_reply(m: Message):
    if not is_admin(m.from_user.id):
        return
...
async def admin_users_from_reply(m: Message):
    if not is_admin(m.from_user.id):
        return
    class _DummyCall:
        def __init__(self, msg):
            self.message = msg
    await send_admin_users_page(_DummyCall(m), 0, banned_only=False)
    await refresh_reply_menu(m.chat.id, m.from_user.id)


@dp.message(F.text == "💰 Прибыль")
async def admin_profit_from_reply(m: Message):
    if not is_admin(m.from_user.id):
        return
