...
def kb_payment(order_id: int, payment_url: Optional[str] = None):
    ...
    rows.append([InlineKeyboardButton(text="🔄 Проверить оплату", callback_data=f"checkpay:{order_id}")])
...
async def reply_admin_btn(m: Message):
    if not is_admin(m.from_user.id):
        return
    await m.answer("⚙️ *Админ-панель SkyWhy*", reply_markup=kb_admin_menu())
    await refresh_reply_menu(m.chat.id, m.from_user.id)
